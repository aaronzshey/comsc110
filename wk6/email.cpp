...
deque <string> sortList(deque <string>& nonDup);
...
int main() {
  ...
  nonDup = sortList(nonDup);
  for (int i = 0; i < nonDup.size(); i++) {
    outFile << nonDup[i] << "\n";
  }
  if (nonDup.size() == 0 ){
    cout << "Sorry, no emails were found in " << inName << "\n";
  } else {
  cout << nonDup.size() << " emails found and written to " << outName << "\n";
  }
...
deque <string> sortList(deque <string>& nonDup) {
  ...
  for (int i = 0; i < nonDup.size(); i++) 
  {
    for (int j = i + 1; j < nonDup.size(); j++) 
    { 
      if (changeCase(nonDup[i]) > changeCase(nonDup[j])) 
      { 
        // swap code 
        string temp = nonDup[i]; 
        nonDup[i] = nonDup[j];   
        nonDup[j] = temp; 
      } // if 
    } // for 
